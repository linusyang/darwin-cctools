...
CFLAGS="-arch armv6 -I./prelib/usr/include -I${SDKROOT}/usr/include -isysroot ${SDKROOT}" \
LDFLAGS="-miphoneos-version-min=3.0 -L./prelib/usr/lib -L${SDKROOT}/usr/lib -isysroot ${SDKROOT}" \
...
${DEVROOT}/usr/bin/g++ -O2 -arch armv6 -o ./_install/usr/bin/ldid ldid/ldid.cpp \
        -isysroot "${SDKROOT}" -miphoneos-version-min=3.0 \
        -L"${SDKROOT}/usr/lib" -I"${SDKROOT}/usr/include" \
        -Ildid -x c ldid/lookup2.c ldid/sha1.c
